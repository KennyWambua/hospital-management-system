...
<input type="hidden" name="price" placeholder="Price" style="width: 68px; padding-top: 6px; padding-bottom: 6px; margin-right: 4px;" />
...
<?php
	...
	$result->bindParam(':p', $p);
		...
		for($i=0; $row = $result->fetch(); $i++){
	?> 
<a rel="facebox" id="cccc" href="checkout4.php?patient_id=<?php echo $_GET['patient_id']?>&transdep=PHARMACY&invoice=<?php echo $_GET['invoice']?>&total=<?php echo$fgfg ?>&cost=<?php echo $fgfg-$fgyg ?>&cashier=<?php echo $_SESSION['SESS_FIRST_NAME']?>&insurance_id=<?php echo $row['insurance_id']; ?>"><input type="button" class="button round blue image-right ic-add text-upper" align="middle"  value="Check Out"/></a>
<?php
	}
	?>
