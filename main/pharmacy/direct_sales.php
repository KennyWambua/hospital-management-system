...
<?php
	...
	$result->bindParam(':p', $p);
		...
		for($i=0; $row = $result->fetch(); $i++){
	?> 
<a rel="facebox" id="cccc" href="checkoutcredit.php?patient_id=<?php echo $_GET['patient_id']?>&invoice=<?php echo $_GET['invoice']?>&total=<?php echo$fgfg ?>&cost=<?php echo $fgfg-$fgyg ?>&cashier=<?php echo $_SESSION['SESS_FIRST_NAME']?>&insurance_id=<?php echo $row['insurance_id']; ?>"><input type="button" class="button round blue image-right ic-add text-upper" align="middle"  value="Check Out"/></a>
<?php
	}
	?>
